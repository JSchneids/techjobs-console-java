...
public class TechJobs {
    ...
    private static void printJobs(ArrayList<HashMap<String, String>> someJobs) {
        ...
        if (someJobs.isEmpty()) {
            System.out.println("No results. Please search again.");
        } else {
            int currentJobsSize = someJobs.size();
            for (int i=0; i < currentJobsSize; i++) {
                HashMap<String, String> jobs = someJobs.get(i);

                System.out.println("\n*****"); // formatting to split up entries
                for (Map.Entry<String, String> job : jobs.entrySet()) {
                    System.out.println(job.getKey() + ": " + job.getValue());  //getValue and getKey
                }
